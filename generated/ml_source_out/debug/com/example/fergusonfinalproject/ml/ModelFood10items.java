...
public final class ModelFood10items {
  @NonNull
  private final Model model;

  private ModelFood10items(@NonNull Context context, @NonNull Model.Options options) throws
      IOException {
    model = Model.createModel(context, "model_Food10Items.tflite", options);
    MetadataExtractor extractor = new MetadataExtractor(model.getData());
  }

  @NonNull
  public static ModelFood10items newInstance(@NonNull Context context) throws IOException {
    return new ModelFood10items(context, (new Model.Options.Builder()).build());
  }

  @NonNull
  public static ModelFood10items newInstance(@NonNull Context context,
      @NonNull Model.Options options) throws IOException {
    return new ModelFood10items(context, options);
  }
